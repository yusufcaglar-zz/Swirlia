...
function isEmailVerified($conn, $id, $username, $email, $reason, $date) {
	...
	if ($query) {
		...
		if ($row_count != 0) {
			//Mail			
			$subject = "Hesabınız Engellendi";
			
			$message_p = "Merhaba, ".$username.",<br /><br />";
			$message_p .= "Az önce hesabınız hesabınız engellendi.<br />";
			$message_p .= "Engel kalkana kadar hesabınıza giriş yapamayacaksınız ve hesabınız aramalarda çıkmayacak. Ayrıca profil fotoğrafınız ve hakkımda yazınız sıfırlanacak.<br />";
			$message_p .= "Engelin sebebi ve yasağın bitiş zamanı aşağıda yazmaktadır.<br /><br />";
			$message_p .= "Sebep: ".$reason."<br />";
			$message_p .= "UTC±00:00 Zamanı ile yasağın bitiş tarihi, ".$date;
			
			send("notify@swirlia.com", $email, $subject, null, "Hesabınız Engellendi ⛔️", $message_p, "3");
			//Mail
