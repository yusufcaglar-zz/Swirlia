...
$sql = 'SELECT id, restriction_deadline FROM restricted_users';
...
if ($query) {
	...
	if (count($result) != 0) {
		for($i = 0; $i < count($result); $i++) {
			if(time() >= strtotime($result[$i]["restriction_deadline"]))
				removeRestriction($conn, $result[$i]["id"]);
		}
	}
}

function removeRestriction($conn, $id) {
	$sql = 'SELECT username, email FROM users WHERE id = :id';
	$query = $conn -> prepare($sql);
	$query -> execute(array(':id' => $id));
	$data = $query -> fetchObject();
	
	isEmailVerified($conn, $id, $data -> username, $data -> email);
	
	$sql = 'DELETE FROM restricted_users WHERE id = :id';
	$query = $conn -> prepare($sql);
	$query -> execute(array(':id' => $id));
	
	$sql = 'UPDATE swirlia_statistics SET `removed_restrictions` = `removed_restrictions` +1 ORDER BY sno DESC LIMIT 1;';
	$query = $conn -> prepare($sql);
	$query -> execute();
}
...
function isEmailVerified($conn, $id, $username, $email) {
	...
	if ($query) {
		...
		if ($row_count != 0) {
			//Mail			
			$subject = "Hesabınızın Engeli Kalktı";
			
			$message_p = "Merhaba, ".$username.",<br /><br />";
			$message_p .= "Az önce hesabınızın engeli kalktı.<br />";
			$message_p .= "Eskiden olduğu gibi siteye giriş sağlayabilirsiniz.<br />";
			$message_p .= "Lütfen kurallara uymaya çalışınız aksi takdirde hesabınız kalıcı olarak silinebilir.";
			
			send("notify@swirlia.com", $email, $subject, null, "Hesabınızın Engeli Kalktı ✅", $message_p, "3");
			//Mail
