...
$success = "\"<div style='background-color: white; margin-bottom: 10px; padding-top: 5em; padding-bottom: 5px;'>\"
				+\"<div id='table'>\"
					+\"<img src='https://swirlia.net/images/email_changed.png' style='object-fit: contain; width: 195px; height: 195px;' />\"
				...
				+\"<label id='status' style='text-align: center; font-family: Georgia, serif; font-size: large;'>İşlem başarılı! E-mail adresiniz değişti.</label>\"
			+\"</div>\"";
...
$invalid_token = "\"<div style='background-color: white; margin-bottom: 10px; padding-top: 5em; padding-bottom: 5px;'>\"
				...
				+\"<label id='status' style='text-align: center; font-family: Georgia, serif; font-size: large;'>E-mail değiştirme kodu geçersiz.</label>\"
			+\"</div>\"";
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
	...
	<html>
		<head>
			...
			<title>Swirlia - Email Değiştirme</title>
			<style>
				body {
					--vw: 10.24px;
					--vh: 6.57px;
	...
	<?php
	if (isset($_GET['email_change_token']) && !empty($_GET['email_change_token'])) {
		$email_change_token = $_GET['email_change_token'];

		if ($db -> checkChangeTokenExist($email_change_token)) {
			if ($db -> changeEmail($email_change_token)) {
				$db -> updateSwirliaStatistics("email_changes", "1");
