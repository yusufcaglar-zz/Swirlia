...
$success = "\"<div style='background-color: white; margin-bottom: 10px; padding-top: 5em; padding-bottom: 5px;'>\"
				+\"<div id='table'>\"
					+\"<img src='https://swirlia.net/images/email_recovered.png' style='object-fit: contain; width: 195px; height: 195px;' />\"
				...
				+\"<label id='status' style='text-align: center; font-family: Georgia, serif; font-size: large;'>İşlem başarılı! Hesabınız artık eski e-mailinize bağlı.</label>\"
			+\"</div>\"";
...
$invalid_token = "\"<div style='background-color: white; margin-bottom: 10px; padding-top: 5em; padding-bottom: 5px;'>\"
				...
				+\"<label id='status' style='text-align: center; font-family: Georgia, serif; font-size: large;'>E-mail kurtarma kodu geçersiz.</label>\"
			+\"</div>\"";
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
	...
	<html>
		<head>
			...
			<title>Swirlia - Email Kurtarma</title>
			<style>
				body {
					--vw: 10.24px;
					--vh: 6.57px;
	...
	<?php
	if (isset($_GET['email_recover_token']) && !empty($_GET['email_recover_token'])) {
		$email_recover_token = $_GET['email_recover_token'];

		if ($db -> checkRecoverTokenExist($email_recover_token)) {
			if ($db -> recoverEmail($email_recover_token)) {
				$db -> updateSwirliaStatistics("email_recoveries", "1");
